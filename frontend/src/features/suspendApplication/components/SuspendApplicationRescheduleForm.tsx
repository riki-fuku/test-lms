...
import { rescheduleSuspendApplication } from '@/features/suspendApplication/api'
import { SuspendApplicationReserveSlotCalendar } from '@/features/suspendApplication/components'
...
export function SuspendApplicationRescheduleForm({ workspaceId, suspendApplicationId }: Props) {
  ...
  const [execute, isLoading] = useAsyncSafeAction(async () => {
    ...
    await rescheduleSuspendApplication(requestData, {
      callbacks: {
        onSuccess: () => {
          addToast({
            title: '再日程調整が完了しました',
            description: '休学申請の再日程調整が完了しました',
            color: 'success',
          })
          setIsRequestComplete(true)
          router.push(`/renewal/user/${workspaceId}/applications/suspend/${suspendApplicationId}`)
        },
        onError: () => {
          addToast({
            title: '再日程調整に失敗しました',
