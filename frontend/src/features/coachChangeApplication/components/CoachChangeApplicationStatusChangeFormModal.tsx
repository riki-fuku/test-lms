...
import type { UpdateCoachChangeApplicationHttpDocument } from '@/features/coachChangeApplication/api'
import { updateCoachChangeApplication } from '@/features/coachChangeApplication/api'
...
export function CoachChangeApplicationStatusChangeFormModal({
  workspaceId,
  coachChangeApplication,
  isOpen,
  onClose,
  ...
  const {
    ...
    defaultValues: {
      statusId: coachChangeApplication.status.value,
      resultStatusId: coachChangeApplication.resultStatus.value,
      changedReason: coachChangeApplication.latestApplicationStatusLog?.reason,
    },
  })

  const submit = async (data: FormValues) => {
    const requestBody: UpdateCoachChangeApplicationHttpDocument['params']['requestBody'] = {
    ...
    await updateCoachChangeApplication(
      {
        pathParams: {
          workspaceId,
          coachChangeApplicationId: coachChangeApplication.id,
        },
        requestBody,
      },
      {
  ...
  return (
    <Modal
      ...
      header={`${coachChangeApplication.user.fullName} コーチ変更申請 内容変更`}
      body={
        <div className='flex flex-col gap-4'>
          <h2 className='font-bold'>生徒名</h2>
          <p>{coachChangeApplication.user.fullName}</p>
