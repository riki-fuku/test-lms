...
import {
  ...
  ReasonRadioGroup,
} from '@/features/cancellationApplication/components'
...
const schema = yup.object().shape({
  reason: yup.string().required('解約をしたい理由を選択してください'),
  otherReason: yup
    .string()
    .when('reason', (reason) => {
    ...
    .defined(),
  detailReason: yup.string().required('解約をしたい理由を入力してください'),
  interviewDate: yup.string().required('解約面談日を選択してください'),
  employeeId: yup.string().required('解約面談日を選択してください'),
})
...
export function CancellationApplicationForm({ onConfirm, workspaceId }: Props) {
  const { formValues, setFormValues } = useCancellationFormStore()
