...
import { rescheduleCancellationApplication } from '@/features/cancellationApplication/api'
import { CancellationApplicationReserveSlotCalendar } from '@/features/cancellationApplication/components'
...
const schema = yup.object().shape({
  interviewDate: yup.string().required('解約面談日を選択してください'),
  isConfirmed: yup
    .boolean()
    .default(false)
...
export function CancellationApplicationRescheduleForm({
  workspaceId,
  ...
  const [execute, isLoading] = useAsyncSafeAction(async () => {
    ...
    await rescheduleCancellationApplication(requestData, {
      callbacks: {
        onSuccess: () => {
          addToast({
            title: '再日程調整が完了しました',
            description: '解約申請の再日程調整が完了しました',
            color: 'success',
          })
          setIsRequestComplete(true)
          router.push(
            `/renewal/user/${workspaceId}/applications/cancellation/${cancellationApplicationId}`,
          )
        },
        onError: () => {
          addToast({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='space-y-10'>
      <Section>
        ...
        <CancellationApplicationReserveSlotCalendar
          name='interviewDate'
          control={control}
