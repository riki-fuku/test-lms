...
export type SlotCalendarProps = {
  ...
  name?: string
  className?: string
  onChangeDate?: (cellItem: CellItem | null) => void
}
  ...
  return (
    <div className='flex items-center py-1'>
      <div className='grid h-10 w-full grid-cols-8 gap-1'>
        <div></div>
        {dates.map((date) => (
          <div key={date} className='flex items-center justify-center gap-1'>
            <div>{dateTools.formatDate(date, 'MM/DD')}</div>
            <div
              className={cn(
  ...
  if (!time.isAvailable) {
    ...
    return (
      <CellBody className='cursor-pointer border bg-white' onClick={onClick}>
        <span className='cursor-pointer bg-gradient-to-r from-sub-color to-main-color bg-clip-text text-3xl text-transparent'>
          ○
        </span>
      </CellBody>
    )
